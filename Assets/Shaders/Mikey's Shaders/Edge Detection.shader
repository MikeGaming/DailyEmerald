Shader "Hidden/Edge Detection"
{
    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            {
                return float2(hash(uv) * sin(uv.y * _OutlineFrequency),
                hash(uv) * cos(uv.x * _OutlineFrequency)) * _OutlineAmplitude /_ScreenParams.xy;
            }
            ...
            half4 frag(Varyings IN) : SV_TARGET
            {
                ...
                float2 displaced_uv = displacement(IN.texcoord);
                // add a bit of wiggle to the uvs
                ...
                float2 uvs[4];
                uvs[0] = uv + displaced_uv + texel_size * float2(half_width_f, half_width_c) * float2(-1, 1);  // top left
                uvs[1] = uv + displaced_uv + texel_size * float2(half_width_c, half_width_c) * float2(1, 1);   // top right
                uvs[2] = uv + displaced_uv + texel_size * float2(half_width_f, half_width_f) * float2(-1, -1); // bottom left
                uvs[3] = uv + displaced_uv + texel_size * float2(half_width_c, half_width_f) * float2(1, -1);  // bottom right
